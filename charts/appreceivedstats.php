...
if (isset($_REQUEST["appid"]) || isset($debugappid)) {
	if (isset($_REQUEST["appid"])) $selected = $_REQUEST["appid"];
	if (isset($debugappid)) $selected = $debugappid;
	...
	arsort($history["queriesReceived"]);
	...
	foreach ($history["queriesReceived"] as $appid => $impressions) {
		try {	
			...
			// Some apps might have been deleted in the meantime, no big deal
		}		
	}
